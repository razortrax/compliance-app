...
export async function GET(
  ...
  try {
    ...
    console.log(`🔍 Fetching MVR data for driver ${driverId} in org ${orgId} under master ${masterOrgId}`)
    ...
    if (!masterOrg) {
      ...
      return NextResponse.json(
        { error: 'Master organization not found' },
        { status: 404 }
      )
    }
    ...
    if (!driver) {
      ...
      return NextResponse.json(
        { error: 'Driver not found in this organization' },
        { status: 404 }
      )
    }
