...
export async function GET(
  ...
  try {
    ...
    const licenseStats = calculateComplianceStats(licenses.filter(l => l.expirationDate) as Array<{ expirationDate: Date }>)
    const mvrStats = calculateComplianceStats(mvrs.filter(m => m.expirationDate) as Array<{ expirationDate: Date }>)
    
    // Training has additional required count
    const trainingStats = calculateComplianceStats(training.filter(t => t.expirationDate) as Array<{ expirationDate: Date }>)
    ...
    return NextResponse.json(
      { error: 'Failed to fetch driver overview data' },
      { status: 500 }
    )
  }
} 