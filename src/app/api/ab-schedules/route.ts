  ...
  try {
    ...
    if (organizationId) {
      if (includeDefaults) {
        ...
        where.OR = [{ organizationId }, { organizationId: null, isDefault: true }];
      } else {
    ...
    const schedules = await db.equipment_ab_schedule.findMany({
      ...
      orderBy: [{ equipmentCategory: "asc" }, { scheduleType: "asc" }, { sortOrder: "asc" }],
    });
  ...
  try {
    ...
    if (
      ...
      return Response.json(
        {
          error:
            "Missing required fields: organizationId, equipmentCategory, scheduleType, itemCode, itemDescription, intervalType, category, component, taskType",
        },
        { status: 400 },
      );
    }
