...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (existingActivity.createdBy !== userId) {
      return NextResponse.json({ error: "Unauthorized to edit this activity" }, { status: 403 });
    }

    // Handle credential encryption for updates
    let encryptedPassword = existingActivity.password;
    if (
      data.activityType === "credential" &&
      data.password &&
      data.password !== existingActivity.password
    ) {
      encryptedPassword = `[ENCRYPTED]${data.password}`; // Placeholder
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (existingActivity.createdBy !== userId) {
      return NextResponse.json({ error: "Unauthorized to delete this activity" }, { status: 403 });
    }
