  ...
  try {
    ...
    if (!incidentId || !violationType || !violationCodes || !organizationId) {
      return NextResponse.json({
        error: 'Missing required fields: incidentId, violationType, violationCodes, organizationId'
      }, { status: 400 })
    }
    ...
    if (!canCreateCAFs || !currentUserContext) {
      return NextResponse.json({
        error: 'User does not have CAF creation permissions. Must be either a Master user or Organization staff with CAF approval rights.'
      }, { status: 403 })
    }
