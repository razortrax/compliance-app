...
import { MvrType, MvrResult, MvrResultDach, MvrResultStatus, MvrCertification, MvrStatus } from '@prisma/client'
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const newMvrIssue = await tx.mvr_issue.create({
        data: {
          ...
          // Set dates
          startDate: body.startDate ? new Date(body.startDate) : (existingMvr.expirationDate || new Date()),
