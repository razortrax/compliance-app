...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!data.partyId || !data.maintenanceType || !data.sourceType || !data.dueDate || !data.workDescription) {
      return Response.json({ 
        error: 'Missing required fields: partyId, maintenanceType, sourceType, dueDate, workDescription' 
      }, { status: 400 })
    }
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const issue = await tx.issue.create({
        data: {
          ...
          title: data.title,
          description: data.description || `${data.maintenanceType} maintenance: ${data.workDescription}`,
          status: 'open',
