...
import { db } from '@/db'
import { MaintenanceType, MaintenanceSourceType, MaintenanceStatus, MaintenanceResult, MaintenancePriority } from '@prisma/client'
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (Object.keys(issueUpdateData).length > 0) {
        ...
        await tx.issue.update({
          where: { 
            id: (await tx.maintenance_issue.findUnique({
              where: { id: params.id },
              select: { issueId: true }
            }))?.issueId 
          },
      ...
      if (data.correctiveActions !== undefined) updateData.correctiveActions = data.correctiveActions as any
