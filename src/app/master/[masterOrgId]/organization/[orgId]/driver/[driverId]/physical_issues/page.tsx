...
function getExpirationStatus(physical: PhysicalIssue): {
  label: string;
  color: string;
  bgColor: string;
} {
  ...
  if (daysUntilExpiration < 0) {
    ...
    return {
      label: `Due in ${daysUntilExpiration} days`,
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/master/${masterOrgId}/organization/${orgId}/driver/${driverId}/physical_issues`,
        );
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500">
                ...
                <span>•</span>
                <span>
                  Active: {sortedPhysicals.filter((p) => p.issue.status === "Active").length}
                </span>
              </div>
            </div>
            ...
            <div className="w-[300px] flex-shrink-0">
              <Card className="h-full">
                <CardHeader>
                  ...
                  <CardDescription>DOT physical examination records</CardDescription>
                </CardHeader>
                ...
                <CardContent className="flex-1 overflow-auto">
                  {selectedPhysical ? (
                    <>
                        ...
                        <div className="space-y-3">
                          ...
                          {selectedPhysical.result && (
                            <div>
                              ...
                              <span className="ml-2">
                                {PHYSICAL_RESULT_LABELS[selectedPhysical.result] ||
                                  selectedPhysical.result}
                              </span>
                            </div>
                          )}
                        ...
                        <div className="space-y-3">
                          {selectedPhysical.startDate && (
                            <div>
                              ...
                              <span className="ml-2">
                                {format(new Date(selectedPhysical.startDate), "MMM dd, yyyy")}
                              </span>
                            </div>
                          )}
                          {selectedPhysical.expirationDate && (
                            <div>
                              ...
                              <span className="ml-2">
                                {format(new Date(selectedPhysical.expirationDate), "MMM dd, yyyy")}
                              </span>
                            </div>
                          )}
                          <div>
                            ...
                            <span className="ml-2">
                              {selectedPhysical.selfCertified ? "Yes" : "No"}
                            </span>
                          </div>
                          <div>
                            <span className="font-medium text-gray-700">National Registry:</span>
                            <span className="ml-2">
                              {selectedPhysical.nationalRegistry ? "Yes" : "No"}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      {(selectedPhysical.status === "Disqualified" ||
                        selectedPhysical.result === "Disqualified") && (
                          ...
                          <div className="grid md:grid-cols-2 gap-4">
                            {selectedPhysical.outOfServiceDate && (
                              <div>
                                ...
                                <span className="ml-2">
                                  {format(
                                    new Date(selectedPhysical.outOfServiceDate),
                                    "MMM dd, yyyy",
                                  )}
                                </span>
                              </div>
                            ...
                            {selectedPhysical.backInServiceDate && (
                              <div>
                                ...
                                <span className="ml-2">
                                  {format(
                                    new Date(selectedPhysical.backInServiceDate),
                                    "MMM dd, yyyy",
                                  )}
                                </span>
                              </div>
                    ...
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 mb-2">
                          Select a physical record
                        </h3>
                        <p className="text-gray-600">
                          Choose a physical examination from the list to view details
                        </p>
                      </div>
                    </div>
                  )}
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>Update the physical examination record</DialogDescription>
              </DialogHeader>
              <div className="px-1">
                {selectedPhysical && (
                  <PhysicalIssueForm
