...
export default function MasterDriverTrainingPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/master/${masterOrgId}/organization/${orgId}/driver/${driverId}/training`,
      );
  ...
  const handleAddTraining = async (formData: any) => {
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : "An error occurred while creating the training",
      );
    } finally {
  ...
  const handleEditTraining = async (formData: any) => {
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : "An error occurred while updating the training",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleRenewTraining = async (formData: any) => {
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : "An error occurred while renewing the training",
      );
    } finally {
      setIsSubmitting(false);
    }
    ...
    const fetchData = async () => {
      try {
        ...
        if (trainingResult.status === "fulfilled" && trainingResult.value.ok) {
          ...
          console.log(
            `📚 Loaded ${pageData.trainings.length} training records for ${pageData.driver.firstName} ${pageData.driver.lastName}`,
          );
        } else {
          const error =
          ...
          <div className="w-[300px] flex-shrink-0">
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>DOT mandatory and voluntary training records</CardDescription>
              </CardHeader>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        {selectedTraining.startDate && (
                          <div>
                            ...
                            <span className="ml-2">
                              {format(new Date(selectedTraining.startDate), "MMM dd, yyyy")}
                            </span>
                          </div>
                        )}
                        <div>
                          ...
                          <span className="ml-2">
                            {format(new Date(selectedTraining.completionDate), "MMM dd, yyyy")}
                          </span>
                        </div>
                        {selectedTraining.expirationDate && (
                          <div>
                            ...
                            <span className="ml-2">
                              {format(new Date(selectedTraining.expirationDate), "MMM dd, yyyy")}
                            </span>
                          </div>
                        )}
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-gray-600">
                        Choose a training from the list to view its details
                      </p>
                    </div>
                  </div>
                )}
