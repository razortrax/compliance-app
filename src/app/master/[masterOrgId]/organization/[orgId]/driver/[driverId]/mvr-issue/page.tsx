...
export default function MVRIssuePage({ params }: MVRIssuePageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`/api/master/${masterOrgId}/organization/${orgId}/driver/${driverId}/mvr-issue`)
  ...
  const handleEditSuccess = (updatedMvr: any) => {
    ...
    setData(prev => {
      ...
      return {
        ...prev,
        mvrIssues: prev.mvrIssues?.map(mvr => 
          mvr.id === updatedMvr.id ? updatedMvr : mvr
        ) || []
      }
    })
  ...
  const getExpirationStatus = (mvr: MVRIssue) => {
    ...
    const hasNewerMVR = data?.mvrIssues?.some(otherMvr => 
      otherMvr.id !== mvr.id && 
        ...
        <div className="flex gap-6 h-[calc(100vh-200px)]">
          ...
          <div className="w-[300px] flex-shrink-0">
            ...
            <CardHeader>
              ...
              <CardDescription>
                Motor vehicle record history for this driver
              </CardDescription>
            </CardHeader>
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        {mvr.cleanRecord ? (
                          ...
                          <p className="text-amber-600 font-medium">⚠ {mvr.violationsCount} Violations</p>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <CardHeader>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <CardDescription>
                    {selectedMVR ? 'Details for selected MVR record' : 'Select an MVR record to view details'}
                  </CardDescription>
                </div>
                {selectedMVR && (
