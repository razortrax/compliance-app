...
export default function LicensesPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'current' | 'warning' | 'critical' | 'expired'>('all')
              ...
              <Card key={license.id} className="hover:shadow-md transition-shadow">
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge className={`${getStatusColor(license.calculatedStatus)} flex items-center gap-1`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{license.licenseState} - {license.licenseNumber}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-gray-400" />
                      <span>Expires: {format(new Date(license.expirationDate), 'MMM dd, yyyy')}</span>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {license.endorsements.map((endorsement, index) => (
                          <Badge key={index} variant="outline" className="text-xs">
                            {endorsement.code} - {ENDORSEMENT_LABELS[endorsement.code] || endorsement.name}
                          </Badge>
                        ))}
                      </div>
                    </div>
