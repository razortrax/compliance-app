...
import { Filter, Plus, Building2, Users } from "lucide-react";
...
export default function OrganizationPartnersPage() {
  ...
  useEffect(() => {
    fetchPartners();
  }, [organizationId, selectedCategory, query, includeInactive]);
  ...
  const topNav = useMemo(
    () => [
      { label: "Organizations", href: "/organizations", isActive: false },
      { label: "Organization", href: `/organizations/${organizationId}`, isActive: false },
      { label: "Partners", href: `/organizations/${organizationId}/partners`, isActive: true },
    ],
    [organizationId],
  );
        ...
        <div className="w-96 border-r border-gray-200 flex flex-col">
          <div className="p-3 border-b flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch("/api/partners", {
                              ...
                              body: JSON.stringify({ organizationId, name: newPartnerName.trim(), category: newPartnerCategory }),
                            });
                            if (res.ok) {
              ...
              <Button size="sm" variant="outline" onClick={() => router.push(`/organizations/${organizationId}`)}>
                Back
              </Button>
            </div>
          </div>
          ...
          <div className="p-3 border-t">
            <Button size="sm" className="w-full" onClick={() => router.push(`/organizations/${organizationId}`)}>
