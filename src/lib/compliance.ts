    ...
    if (rule.triggerType === 'license_endorsement') {
      ...
      if (licensesWithEndorsement.length > 0) {
        ...
        const relevantTrainings = trainings.filter(training => 
          training.trainingType === requiredTrainingType &&
          training.issue?.status === 'active'
        )
        ...
        if (relevantTrainings.length === 0) {
          ...
          const latestTraining = relevantTrainings.sort((a, b) => 
...
export function getComplianceActionPriority(status: ComplianceCheck['status']): number {
  switch (status) {
    case 'expired': return 1 // Highest priority
    case 'missing': return 2
    case 'expiring_soon': return 3
    case 'compliant': return 4 // Lowest priority
    default: return 5
  }
} 