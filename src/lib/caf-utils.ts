...
export function groupViolationsByType(violations: any[]): ViolationGroup[] {
  ...
  const driverViolations = violations.filter(
    (v) => v.violationType === "Driver_Qualification" || v.violationType === "Driver_Performance",
  );
...
export async function findAssignableStaff(
  ...
  const response = await fetch(
    `/api/staff?organizationId=${organizationId}&cafAssignment=${violationType}`,
  );
  if (!response.ok) {
 ...
 */
export function generateCAFTitle(
  violationType: "Driver" | "Equipment" | "Company",
  violations: any[],
): string {
...
export async function createCAF(request: CAFGenerationRequest): Promise<any> {
  const priority = calculateCAFPriority(
    request.violationCodes.map((code) => ({ violationCode: code })),
  );
  ...
  const response = await fetch("/api/corrective-action-forms", {
    ...
    body: JSON.stringify({
      ...
      title: generateCAFTitle(
        request.violationType,
        request.violationCodes.map((code) => ({ violationCode: code })),
      ),
      description: generateCAFDescription(
        request.violationCodes.map((code) => ({ violationCode: code })),
      ),
    }),
  });
  ...
  try {
    ...
    const allComplete = cafs.every((caf: any) => {
      ...
      const hasCompletionSignature = caf.signatures?.some(
        (sig: any) => sig.signatureType === "COMPLETION",
      );

      // For approval, either have approval signature OR be auto-approved by master
      const hasApprovalOrAutoApproved =
        caf.signatures?.some((sig: any) => sig.signatureType === "APPROVAL") || caf.approvedAt; // Auto-approved by master user
