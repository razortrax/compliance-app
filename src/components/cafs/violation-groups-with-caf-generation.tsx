  ...
  function getExistingCAF(violationType: string): ExistingCAF | undefined {
    return existingCAFs.find((caf) => {
      ...
      if (violationType === "Driver") {
        return (
          caf.violationType === "Driver_Performance" || caf.violationType === "Driver_Qualification"
        );
      }
  ...
  if (violationGroups.length === 0) {
    return (
      <Alert>
        ...
        <AlertDescription>No violations found that require corrective action.</AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h4 className="font-medium text-gray-900">
          Corrective Action Forms ({violationGroups.length} type
        ...
        <Button
          ...
          disabled={
            violationGroups.every((group) => hasCAFGenerated(group.type)) || generatingCAF !== null
          }
        >
            ...
            {isExpanded && (
              <CardContent className="space-y-3">
                {group.violations.map((violation) => (
                  <div
                    key={violation.id}
                    className="bg-white/70 border border-white/50 rounded-lg p-3"
                  >
