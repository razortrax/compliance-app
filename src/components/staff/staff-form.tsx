...
export const StaffForm: React.FC<StaffFormProps> = ({
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    hireDate: staff?.role?.startDate
      ? new Date(staff.role.startDate).toISOString().split("T")[0]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        `Error saving staff member: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
      setLoading(false);
    }
  ...
  const availablePersonsForSelection = availablePersons.filter(
    (person) =>
    ...
    <div className="space-y-6 max-h-[70vh] overflow-y-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <Tabs
            value={personTab}
            onValueChange={(value) => setPersonTab(value as "existing" | "new")}
            className="mt-2"
          >
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="existing">Select Existing</TabsTrigger>
              <TabsTrigger value="new" disabled={!!staff}>
                Create New
              </TabsTrigger>
            </TabsList>
        ...
        <div>
          ...
          <div className="mt-3 space-y-3">
            ...
            <div className="text-sm text-gray-600 p-3 bg-gray-50 rounded">
              <strong>Note:</strong> CAF permissions are used for the corrective action workflow in
              roadside inspections and accidents. Staff with signing permissions can complete
              assigned CAFs, while staff with approval permissions can review and approve completed
              CAFs.
            </div>
          </div>
        </div>
