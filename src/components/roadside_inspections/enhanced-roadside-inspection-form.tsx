...
export default function EnhancedRoadsideInspectionForm({
      ...
      try {
        ...
        if (masterOrgId && organizationId) {
          ...
          if (equipmentResponse.status === "fulfilled" && equipmentResponse.value.ok) {
            ...
            const equipment =
              equipmentData.equipment?.map((equip: any) => ({
                id: equip.id,
  ...
  const updateViolation = (violationId: string, field: keyof Violation, value: any) => {
    setFormData((prev) => ({
      ...prev,
      violations: prev.violations.map((v) => (v.id === violationId ? { ...v, [field]: value } : v)),
    }));
  };
                          ...
                          <div key={equipment.id} className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => {
                                if (checked) {
                                  handleInputChange("selectedEquipmentIds", [
                                    ...formData.selectedEquipmentIds,
                                    equipment.id,
                                  ]);
                                } else {
                                  handleInputChange(
                                    "selectedEquipmentIds",
                                    formData.selectedEquipmentIds.filter(
                                      (id) => id !== equipment.id,
                                    ),
                                  );
                                }
            ...
            <div className="p-6 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Search and add violations found during inspection
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                  ...
                  {availableViolations.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-sm text-gray-500">
                        Found {availableViolations.length} violations:
                      </div>
                      {availableViolations.map((violation) => (
                        ...
                        >
                          ...
                          <Badge
                            variant={
                              violation.severity === "OUT_OF_SERVICE" ? "destructive" : "secondary"
                            }
                          >
                            {violation.severity}
                          </Badge>
                  ...
                  {violationSearch.length >= 2 && availableViolations.length === 0 && (
                    <div className="text-sm text-gray-500">
                      No violations found for "{violationSearch}"
                    </div>
                  )}
                        ...
                        <Card key={violation.id}>
                          <CardContent className="pt-4">
                            ...
                            <div className="space-y-3">
                              <div>
                                <Label htmlFor={`comments-${violation.id}`}>
                              ...
                              {(() => {
                                ...
                                if (
                                  ...
                                  updateViolation(
                                    violation.id,
                                    "unitNumber",
                                    formData.selectedEquipmentIds[0],
                                  );
                                }
                                ...
                                return shouldShowSelector ? (
                                  <div>
                                    ...
                                    <Select
                                      ...
                                      onValueChange={(value) =>
                                        updateViolation(violation.id, "unitNumber", parseInt(value))
                                      }
                                    >
                                      <SelectTrigger>
                                        ...
                                        {formData.selectedEquipmentIds.map((equipmentId) => {
                                          ...
                                          return (
                                            <SelectItem key={equipmentId} value={equipmentId}>
                                              Unit {equipment?.unitNumber || "N/A"} -{" "}
                                              {equipment?.vehicleType || "Unknown"}
                                              {(equipment?.make || equipment?.model) &&
                                    ...
                                    <div className="text-sm text-green-700 bg-green-50 p-2 rounded">
                                      ...
                                      {formData.selectedEquipmentIds[0]
                                        ? availableEquipment.find(
                                            (e) => e.id === formData.selectedEquipmentIds[0],
                                          )?.unitNumber
                                        : "N/A"}{" "}
                                      -{" "}
                                      {formData.selectedEquipmentIds[0]
                              ...
                              <div className="flex items-center space-x-2">
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) =>
                                    updateViolation(violation.id, "outOfService", checked)
                                  }
                                />
                ...
                <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      <p>
                        {formData.incidentDate
                          ? format(new Date(formData.incidentDate), "MMMM d, yyyy")
                          : "Not set"}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p>
                        {formData.primaryDriverId
                          ? getDriverName(formData.primaryDriverId)
                          : "Not selected"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p>
                        {formData.isCoDriverPresent && formData.coDriverId
                          ? getDriverName(formData.coDriverId)
                          : "None"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    <span className="font-medium">
                      Equipment ({formData.selectedEquipmentIds.length}):
                    </span>
                        ...
                        {formData.violations.map((violation) => (
                          ...
                          >
                            <span className="text-sm">
                              {violation.violationCode} - {violation.description}
                            </span>
