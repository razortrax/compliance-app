  ...
  try {
    ...
    for (const org of organizations) {
      ...
      if (equipment.length > 0) {
        equipment.forEach((eq, index) => {
          console.log(
            `      ${index + 1}. ${eq.vehicleTypeId || "No Type"} - ${eq.make} ${eq.model} (${eq.year})`,
          );
        });
      }
    }
    ...
    if (orphanedPeople.length > 0) {
      ...
      orphanedPeople.forEach((person, index) => {
        console.log(
          `   ${index + 1}. ${person.firstName} ${person.lastName} (Party ID: ${person.partyId})`,
        );
      });
    }
    ...
    if (orphanedEquipment.length > 0) {
      ...
      orphanedEquipment.forEach((eq, index) => {
        console.log(
          `   ${index + 1}. ${eq.vehicleTypeId || "No Type"} - ${eq.make} ${eq.model} (Party ID: ${eq.partyId})`,
        );
      });
    }
