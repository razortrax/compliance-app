...
async function cleanupOrphanedData() {
  ...
  try {
    ...
    const masterParty = userParties.find(
      (party) => party.person && party.role.some((role) => role.roleType === "master"),
    );

    if (masterParty) {
      console.log(
        `✅ Keeping master party: ${masterParty.id} (${masterParty.person?.firstName} ${masterParty.person?.lastName})`,
      );

      // Find orphaned parties (no person or no master role)
      const orphanedParties = userParties.filter(
        (party) =>
          party.id !== masterParty.id &&
