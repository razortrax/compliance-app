...
async function checkAllData() {
  try {
    ...
    allPeople.forEach((person, index) => {
      console.log(
        `  ${index + 1}. ${person.firstName} ${person.lastName} (Status: ${person.party?.status || "Unknown"})`,
      );
    ...
    allEquipment.forEach((eq, index) => {
      console.log(
        `  ${index + 1}. Vehicle Type: ${eq.vehicleTypeId || "None"} - ${eq.make} ${eq.model} (Status: ${eq.party?.status || "Unknown"})`,
      );
    ...
    allParties.forEach((party, index) => {
      console.log(
        `  ${index + 1}. Party ID: ${party.id} (Status: ${party.status}, User: ${party.userId || "None"})`,
      );
    });
